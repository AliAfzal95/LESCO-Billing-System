...
public:
	...
	static int StringLenght(char* str1)
	{
		int str_length = 0;
	...
	// returns the required date which user enter
	static int getCurrentDate(int dueDate_Check = 0)
	{
		time_t t = time(0);
		tm now;
		localtime_s(&now, &t);
...
void Tarrif::updateTarrifFile()
{
	...
	// getting taarif info through txt file
...
protected:
	...
	int fixed_Charges;
	int billing_Month;
	int reading_EntryDate;
	double sales_TaxAmount;
	double electricity_Cost;
	double totalBilling_Amount;
	double currentMeter_Regular;
	// deceleration of function in public
public:
	Billing();
	~Billing();
	char* getBillerID();
...
Billing::Billing()
{
	...
	sales_TaxAmount = 0;
	electricity_Cost = 0;
	totalBilling_Amount = 0;
	currentMeter_Regular = 0;
	bill_Status = biller_ID = nullptr;
}
...
void Billing::addNewBillingInfo(char* ID, char* customertype, char* metertype)
{
	...
	int month = (now.tm_mon + 1);
	string Month;
	// if statement to check the month
	if (month < 10)
		Month = "0" + to_string(month);
	else
	{
	...
	billing_Month = stoi(Month);
	currentMeter_Regular = 0;
	reading_EntryDate = Helper::getCurrentDate();
	totalBilling_Amount = 0;
	due_Date = Helper::getCurrentDate(1);
	Tarrif TarrifInfo;
	...
	{
		electricity_Cost = TarrifInfo.phase1_dom_reg_Price;
		fixed_Charges = TarrifInfo.phase1_dom_fixedCharges;
		sales_TaxAmount = TarrifInfo.phase1_dom_taxPercentage;
	}
	else if (customertype[0] == 'c' && metertype[0] == '1')
	{
		electricity_Cost = TarrifInfo.phase1_com_reg_Price;
		fixed_Charges = TarrifInfo.phase1_com_fixedCharges;
		sales_TaxAmount = TarrifInfo.phase1_com_taxPercentage;
	}
	paid_Date = 0;
	bill_Status == "0";
}
// reads existing customers billing information from file and sets variables
void Billing::loadBillingInfoFromFile(ifstream& fin, char* biller_ID1)
{
	...
	fin.ignore();
	fin.ignore();
	fin >> reading_EntryDate;
	fin.ignore();
	fin >> electricity_Cost;
	fin.ignore();
	fin.ignore();
	fin >> sales_TaxAmount;
	fin.ignore();
	fin >> fixed_Charges;
	fin.ignore();
	fin >> totalBilling_Amount;
	fin.ignore();
	fin >> due_Date;
	fin.ignore();
	char tempBillStatus[10];
	fin.getline(tempBillStatus, 100, ',');
	bill_Status = Helper::GetStringFromBuffer(tempBillStatus);
	fin >> paid_Date;
}
...
void Billing::setStatus(char* status, double& regUnits, double& peakUnits)
{
	...
	{
		bill_Status = status;
		paid_Date = Helper::getCurrentDate();
		regUnits = currentMeter_Regular;
		peakUnits = 0;
		reading_EntryDate = 0;
		due_Date = 0;
...
void Billing::calculateBill(double regUnits, double peakUnits)
{
	...
	double costOfElectricity = unitsConsumed * electricity_Cost;
	costOfElectricity += costOfElectricity * (sales_TaxAmount / 100.0);
	costOfElectricity += fixed_Charges;
	totalBilling_Amount = costOfElectricity;
}
// incase of new addition or change, updation of file with new values
void Billing::updateBillingFile(ofstream& fout)
{
	fout << biller_ID << "," << billing_Month << "," << currentMeter_Regular << ",," << reading_EntryDate << "," << electricity_Cost << ",,"
		<< sales_TaxAmount << "," << fixed_Charges << "," << totalBilling_Amount << "," << due_Date << "," << bill_Status << "," << paid_Date;
}
// interface print for billing month, meter type, costs, taxes, and percentages
void Billing::print()
{
	cout << "Billing Month:   " << billing_Month << endl;
	cout << "Current Meter:   " << currentMeter_Regular << endl;
	cout << "Entry Date:   ";
	Helper::displayDate(reading_EntryDate);
	cout << endl;
	cout << "Electricity Cost:" << electricity_Cost << endl;
	cout << "Sales Tax Amount:" << sales_TaxAmount << endl;
	cout << "Fixed Charges:" << fixed_Charges << endl;
	cout << "Total Amount:" << totalBilling_Amount << endl;
	cout << "Due Date:";
	Helper::displayDate(due_Date);
...
private:
	Billing* biller;
	char* meterType; int customerDOB;
	char* customerID;
	int connectionDate;
	char* customerName;
	char* customerType;
	...
	// decelration of function in public 
public:
	Customer();
	~Customer();
	int getPeakUnits();
...
void Customer::addNewCustomer(Customer**& List, int total)
{
	...
	while (!flag)
	{
		...
		// apllying stiwch statment check cousmtomer 
		switch (_getch())
		{
		case '1':
	...
	while (!flag)
	{
		...
		int i;
		sprintf_s(tempID, sizeof(tempID), "%d", number);
...
void phase3Meter::addNewBillingInfo(char* ID, char* customertype, char* metertype)
{
	...
	reading_EntryDate = Helper::getCurrentDate();
	totalBilling_Amount = 0;
	due_Date = Helper::getCurrentDate();
	Tarrif TarrifInfo;
	ifstream readTax("TarrifTaxInfo.txt");
	TarrifInfo.LoadTarrifTaxInfo(readTax);
	readTax.close();
	if (customertype[0] == 'd' && metertype[0] == '3')
	{
		...
		electricity_Cost = TarrifInfo.phase3_dom_reg_Price;
		sales_TaxAmount = TarrifInfo.phase3_dom_taxPercentage;
		fixed_Charges = TarrifInfo.phase3_dom_fixedCharges;
	}
	else if (customertype[0] == 'c' && metertype[0] == '3')
	{
		electricityCost_Peak = TarrifInfo.phase3_com_peak_Price;
		electricity_Cost = TarrifInfo.phase3_com_reg_Price;
		sales_TaxAmount = TarrifInfo.phase3_com_taxPercentage;
		fixed_Charges = TarrifInfo.phase3_com_fixedCharges;
	}
	paid_Date = 0;
	bill_Status == "0";
}
void phase3Meter::loadBillingInfoFromFile(ifstream& fin, char* biller_ID1)
{
	biller_ID = biller_ID1;
	fin >> billing_Month;
	fin.ignore();
...
{
	fout << biller_ID << "," << billing_Month << "," << currentMeter_Regular << "," << currentMeter_Peak
		<< "," << reading_EntryDate << "," << electricity_Cost << "," << electricityCost_Peak << ","
		<< sales_TaxAmount << "," << fixed_Charges << "," << totalBilling_Amount << "," << due_Date << ","
		<< bill_Status << "," << paid_Date;
}
void phase3Meter::calculateBill(double regUnits, double peakUnits)
{
	...
	double costOfRegElectricity = regUnitsConsumed * electricity_Cost;
	...
	costOfElectricity += costOfElectricity * (sales_TaxAmount / 100);
	costOfElectricity += fixed_Charges;
	totalBilling_Amount = costOfElectricity;
}
...
bool Employee::setBillStatus(Customer*& customer)
{
	...
	while (!flag)
	{
		...
		{
		case '1':
			strcpy_s(billstatus, sizeof(billstatus), "paid");
			flag = true;
			break;
		case '2':
			billstatus[0] = '0';
...
int main()
{
	...
	Obj.AssociateBills();
	// intilization of variable in main
	bool flag = false;
	int userType = 0;
	// cout statemnet
